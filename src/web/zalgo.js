...
function draw_zalgo_table(elid) {
    ...
    html += '<tr>\n';
    for (let i = 0; i < zalgo_up.length; i++) {
        if (!(i % 10))
            html += '</tr><tr>';
    ...
    html += '<table class="zalgo_ref_table">\n';
    html += '<tr>\n';
    for (let i = 0; i < zalgo_mid.length; i++) {
        if (!(i % 10))
            html += '</tr><tr>';
    ...
    html += '<table class="zalgo_ref_table">\n';
    html += '<tr>\n';
    for (let i = 0; i < zalgo_down.length; i++) {
        if (!(i % 10))
            html += '</tr><tr>';
...
function zalgo_textarea(id) {
    ...
    let container = document.getElementById('lulz_container');
    while (container.childNodes.length)
        container.removeChild(container.childNodes[0]);

    //build blocks for each line & create a <br />
    let lines = newtxt.split("\n");
...
export function zalgo_text(text) {
    let newtxt = '';

    for (let i = 0; i < text.length; i++) {
        if (is_zalgo_char(text.substring(i, i + 1))) // Skip zalgo chars
            continue;

        // Copy character
        newtxt += text.substring(i, i + 1);

        // Options
        let num_up = rand(16) / 2 + 1
        let num_mid = rand(6) / 2
        let num_down = rand(16) / 2 + 1
